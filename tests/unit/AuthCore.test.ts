...
describe('AuthCore', () => {
  ...
  it('should generate PKCE challenge in auth URL', async () => {
    ...
    // GitHub is OAuth2, not OIDC, so no nonce should be present
    expect(authUrl).not.toContain('nonce=');
  ...
  it('should throw TokenRefreshError on refresh failure (Bug #2 fix)', async () => {
    ...
    await expect(authCore.refreshToken('github', 'bad_token')).rejects.toThrow(TokenRefreshError);

    await expect(authCore.refreshToken('github', 'bad_token')).rejects.toThrow(
