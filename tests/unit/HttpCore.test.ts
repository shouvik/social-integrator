...
import { CircuitBreakerOpenError, ApiClientError, ApiServerError, NetworkTimeoutError } from '../../src/utils/errors';
...
describe('HttpCore', () => {
  ...
  it('should send If-None-Match header when ETag cached', async () => {
    ...
    nock('https://api.github.com')
      .get('/test')
      .reply(200, { data: 'first' }, { ETag: '"abc123"' });
  ...
  it('should handle 304 Not Modified correctly', async () => {
    ...
    nock('https://api.github.com')
      .get('/test')
      .reply(200, { data: 'original' }, { ETag: '"v1"' });
    ...
    expect(mockMetrics.incrementCounter).toHaveBeenCalledWith('http_cache_hits', { provider: 'github' });
  });
  ...
  it('should respect rate limits', async () => {
    ...
    await Promise.all(
      Array.from({ length: 10 }, () =>
        httpCore.get('https://api.github.com/test', {})
      )
    );
  ...
  it('should throw ApiClientError for 4xx errors', async () => {
    ...
    await expect(httpCore.get('https://api.github.com/test', {}))
  ...
  it('should extract provider from URL correctly', async () => {
    const testCases = [
      { url: 'https://api.github.com/test', expected: 'github' },
      { url: 'https://googleapis.com/test', expected: 'google' },
      { url: 'https://oauth.reddit.com/test', expected: 'reddit' },
      { url: 'https://api.twitter.com/test', expected: 'twitter' }
    ];
    
    for (const { url, expected } of testCases) {
      nock(url).get('/test').reply(200, {});
      
      await httpCore.get(`${url}/test`, {});
