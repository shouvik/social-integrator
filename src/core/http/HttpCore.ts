...
export class HttpCore {
  ...
  async request<T = unknown>(config: HttpRequestConfig): Promise<HttpResponse<T>> {
    ...
    // Execute request (with rate limiting) - v1.1 FIX #4
    const execute = async (): Promise<HttpResponse<T>> => {
      ...
      try {
        ...
        this.metrics.recordLatency(
          ...
          Date.now() - startTime,
          { provider, status: axiosResponse.status }
        );
  ...
  /**
   * CRITICAL v1.1 FIX #4: Actually execute task in queue
   */
  ...
  private transformError(error: any, provider: ProviderName): Error {
    if (error.response) {
      ...
      if (status >= 400 && status < 500) {
        return new ApiClientError(`Client error: ${status}`, { provider, status, response: error.response.data });
      }
      if (status >= 500) {
