...
export abstract class BaseConnector implements Connector {
  ...
  private async refreshWithDedup(userId: string, refreshToken: string): Promise<TokenSet> {
    ...
    if (!acquired) {
      this.deps.logger.debug('Another instance refreshing, waiting', { provider: this.name, userId });
      this.deps.metrics.incrementCounter('token_refresh_dedup_distributed', { provider: this.name });
  ...
  private async executeRefresh(
    ...
    try {
      ...
      this.deps.metrics.recordLatency(
        ...
        Date.now() - startTime,
        { provider: this.name, status: 'success' }
      );
      ...
      const errorCode = (error instanceof Error && 'code' in error) ? String((error as Record<string, unknown>).code) : 'unknown';
